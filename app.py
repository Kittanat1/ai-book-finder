...
def search_with_ir(query, books, doc_vectors, vocab_index, idf):
    """ค้นหาด้วย TF-IDF + Cosine Similarity + Field Boosting"""
...
def recommend_similar(book_idx, books, doc_vectors, top_n=4):
    """แนะนำหนังสือที่คล้ายกันด้วย Cosine Similarity"""
...
if not st.session_state.loaded_initial:
    with st.spinner('กำลังโหลดหนังสือและสร้าง IR Index...'):
        queries = [
...
with col1:
    query = st.text_input("🔍 ค้นหาด้วย AI (TF-IDF Ranking)", placeholder="พิมพ์คำค้นหา...")
...
with col3:
    sort_by = st.selectbox("เรียง", ["IR Score", "คะแนน", "ปีล่าสุด"])
...
with col_search:
    search_clicked = st.button("🔎 ค้นหาด้วย IR", use_container_width=True)
...
if search_clicked and query:
    with st.spinner('กำลังค้นหาด้วย TF-IDF...'):
        # ใช้ IR แทน string matching
